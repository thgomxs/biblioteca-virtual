...
export const getBookAPI = async (id: string) => {
  ...
  try {
    ...
    book.category = data.volumeInfo.categories
  ...
  try {
    ...
    if (query == '') {
      response = await axios.get(
        'https://www.googleapis.com/books/v1/volumes?q=subject:fiction&orderBy=relevance&maxResults=9',
      );
    } else {
      response = await axios.get(
        `https://www.googleapis.com/books/v1/volumes/?q=intitle:${query}&maxResults=9`,
      );
    }
