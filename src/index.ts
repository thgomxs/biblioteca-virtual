...
io.on("connection", async (socket) => {
  ...
  async function sendReviews(bookID: number, type: string) {
    const book = await bookRepo.findOne({ where: { id: bookID }, relations: ["reviews", "reviews.user"] });
  ...
  socket.on("client:authUser", async (data) => {
    ...
    if (user && bcrypt.compareSync(data.password, user.password)) {
      ...
      return socket.emit("server:authMessage", { message: "Usuário logado!", auth: "login", type: "success", token: token });
    } else {
      return socket.emit("server:authMessage", { message: "Usuário ou senha incorretos!", auth: "login", type: "danger" });
    }
  });

  socket.on("client:newUser", async (data) => {
    if (data.password !== data.confirmPassword) {
      return socket.emit("server:authMessage", { message: "Senhas divergem.", auth: "register", type: "danger" });
    }
    ...
    try {
      ...
      return socket.emit("server:authMessage", { message: "Registrado com sucesso!.", auth: "register", type: "success" });
    } catch (error) {
      return socket.emit("server:authMessage", { message: "Usuário existente.", auth: "register", type: "danger" });
    }
  });
  ...
  socket.on("client:newReview", async ({ comment, bookID, rating }) => {
    ...
    if (userAuthenticated) {
      try {
        ...
        return socket.emit("server:reviewMessage", { message: "Avaliação enviada com sucesso!", type: "danger" });
      } catch (error) {
        return socket.emit("server:reviewMessage", { message: "Erro ao enviar avaliação, tente novamente.", type: "danger" });
      }
    }
  });
...
app.get("/:id", async (req: any, res: any) => {
  ...
  const book = await bookRepo.findOne({ where: { id: req.params.id } });

  if (book) {
    res.render("pages/book", { book: book, user: user ? user : false });
  } else {
    res.redirect("/");
  }
});
